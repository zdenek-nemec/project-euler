...
Description:
    ...
    In the :math:`20 \\times 20` grid below, four numbers along a diagonal
...
def solve_brute_force(numbers, sequence):
    """
    Go through all values per row, then per column, then per diagonal,
    calculate product and select the greatest.
    """
    rows = len(numbers)
    cols = len(numbers[0])
    for row in numbers:
    ...
    # Scan diagonal from top-left to bottom-right
    for y in range(0, rows):
        for x in range(0, cols):
            prod = 1
            for i in range(0, sequence):
                if (((x+i) >= cols) or ((y+i) >= rows)):
                    prod = 0
                    break
                prod *= numbers[y+i][x+i]
            if (prod > max_prod):
                max_prod = prod

    # Scan diagonal from top-right to bottom-left
    for y in range(0, rows):
        for x in range(0, cols):
            prod = 1
            for i in range(0, sequence):
