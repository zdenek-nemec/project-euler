...
def solve_brute_force(number, digits):
    """
    Calculate product for every sequence and store the beggest one.
    """
    max_product = 0
    i = 0
    while (1):
        if ((i+digits) > len(number)):
            break
        product = number[i]
        for j in range(1, digits):
            if (number[i+j] == 0):  # Optimisation: zero product
                product = 0
                break
            product *= number[i+j]
...
def main():
    # Load the number from number.txt file
    number = []
